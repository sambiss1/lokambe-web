  ...
  return (
    <ul className={cx('grid gap-4', columns === 3 ? 'sm:grid-cols-2 lg:grid-cols-3' : 'sm:grid-cols-2')}>
      {items.map((item, index) => (
        ...
        >
          {numbered ? (
            <span className={cx('text-5xl leading-none font-extrabold tabular-nums', dark ? 'text-lokambe-peach' : 'text-lokambe-red')}>
          ...
          <h3 className={cx('mt-6 text-2xl leading-tight font-extrabold uppercase', dark ? 'text-white' : 'text-lokambe-blue')}>
            {item.title}
          </h3>
          <p className={cx('mt-3 text-lg leading-relaxed', dark ? 'text-white/75' : 'text-ink-soft')}>{item.text}</p>
        </Reveal>
      ))}
    </ul>
  );
