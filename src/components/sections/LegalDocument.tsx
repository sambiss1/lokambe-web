  ...
  return (
    <>
      <section className="bg-lokambe-blue pt-32 pb-16 text-white sm:pt-40 sm:pb-20">
        <Container>
          <h1 className="display text-[clamp(1.95rem,4.92vw,3.7rem)]">
            <span className="line-mask">
              <span>{document.title}</span>
            </span>
          </h1>
          <p className="fade-up mt-6 text-[1.0625rem] text-white/75">
          ...
          <div className="space-y-14 lg:col-span-8">
            {document.sections.map((section) => (
              <Reveal as="article" key={section.title} id={slug(section.title)}>
                <h2 className="display text-[clamp(1.3rem,2.46vw,1.85rem)] text-lokambe-blue">{section.title}</h2>
